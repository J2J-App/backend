...
const branches = asyncHandler(async (req, res) => {
    try {
        const { college } = req.body;
        const filePath = path.join(process.cwd(), `src/data-about/josaa_unique_code_all.json`);
        const jsonData = await fs.readFile(filePath, 'utf8');
        const data = JSON.parse(jsonData);
        ...
        return res.status(200).json(
            new ApiResponse(200, final_data, "College branches fetched successfully")
        );
...
export { about,photo,seat_matrix,branches };
